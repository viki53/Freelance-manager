            ...
            <div class="mb-3 overflow-hidden shadow-xl sm:rounded-lg">
                <a href="{{ route('invoices.show', ['invoice' => $invoice]) }}" class="block p-6 bg-white">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4 text-lg text-gray-600 leading-7 font-semibold"><strong>{{ $invoice->customer->name }}</strong> — {{ ($invoice->created_at ?: $invoice->sent_at)->isoFormat('D MMMM YYYY \à HH\hmm') }}</div>
                        @else
                        <div class="ml-4 text-lg text-gray-600 leading-7 font-semibold"><em>Facture</em> — {{ ($invoice->created_at ?: $invoice->sent_at)->isoFormat('D MMMM YYYY \à HH\hmm') }}</div>
                        @endif
                    </div>
